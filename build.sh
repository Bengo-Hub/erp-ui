#!/usr/bin/env bash
# =============================================================================
# ERP UI (Next.js) Build and Deploy Script
# =============================================================================
# Purpose: Builds the Next.js standalone Docker image, pushes to registry, and
#          updates Helm values (apps/erp-ui/values.yaml image.tag) -> ArgoCD.
# Mirrors inventory-service/inventory-ui/build.sh (Next.js service patterns).
# Usage:
#   ./build.sh                     # Build only
...
DOCKER_BUILDKIT=1 docker build . -t "${IMAGE_REPO}:${GIT_COMMIT_ID}" \
  ...
  --build-arg NEXT_PUBLIC_SUBSCRIPTIONS_API_URL="$NEXT_PUBLIC_SUBSCRIPTIONS_API_URL"
